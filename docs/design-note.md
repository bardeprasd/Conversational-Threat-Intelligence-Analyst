...
Multi-turn references are resolved by loading recent ChatKit thread items and passing that transcript to each Agents SDK run. Consequently, “its ASN” and “that IP” can resolve to an IOC established earlier in the same thread. The assessment store is intentionally in memory; durable storage is a production extension.
